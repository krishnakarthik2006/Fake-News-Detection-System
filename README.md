...
The app has been updated to run with the Python standard library only, so you do not need to install Flask, scikit-learn, pandas, or NLTK just to use it.

The current version uses a hybrid verification flow:

1. Google Fact Check Tools API when `GOOGLE_FACT_CHECK_API_KEY` is configured.
2. Wikipedia reference summaries and Wikidata entity matches.
3. Semantic and temporal contradiction checks on the returned evidence.
4. A lightweight local classifier as a fallback signal when live evidence is weak or unavailable.
...
```
python prediction.py --offline
```

Analyze an article URL from the terminal:

```
python prediction.py --url https://example.com/news-story
```
...
1. Split longer input into multiple claims and verify each one separately.
2. Accept an article URL and extract readable text before checking it.
3. Show contradiction highlights such as year mismatches and conflicting numeric values when reference material supports that comparison.
4. Suggest India-specific official sources such as PIB Fact Check, myScheme, RBI, MoHFW, and ECI when the claim looks India-focused.

### Optional API setup

To enable live Google fact-check reviews, create a Google Fact Check Tools API key and set it before starting the app.
